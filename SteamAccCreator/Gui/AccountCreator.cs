...
namespace SteamAccCreator.Gui
{
    ...
    public class AccountCreator
    {
        ...
        public async void Run()
        {
            ...
            if (_mainForm.RandomPass)
            {
                if(_mainForm.NeatPassword){
                    ...
                    //_pass = new CultureInfo("en-US").TextInfo.ToTitleCase(adj[GetRandomNumber(0, adj.Length - 1)]) + new CultureInfo("en-US").TextInfo.ToTitleCase(adj[GetRandomNumber(0, adj.Length - 1)]) + new CultureInfo("en-US").TextInfo.ToTitleCase(animals[GetRandomNumber(0, animals.Length - 1)])+ GetRandomString(2);
        ...
        {
            while (!_httpHandler.CompleteSignup(_alias, _pass, ref _status,_mainForm.UseCsgo))
            {
                UpdateStatus();
                switch (_status)
                {
